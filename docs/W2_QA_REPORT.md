...
- ✅ **RESOLVED (2026-05-09).** `compareLiveAgainstBaseline` now computes a `RegressionSlice[]` (one entry per baselined `(dataset, rubric)` pair) alongside the pooled rate; `underTolerance` is the conjunction of pooled-under, every-slice-under, no-drift, no-removed-cases. The markdown report gained a "Per-(dataset, rubric) slice regressions" section listing only the failing slices with their per-slice rates. Pinned test covers the H7 worked example: a 5/110 = 4.5% pooled rate that hides a 5/5 = 100% slice on a small dataset → gate fails with the slice as the named cause. Boundary test rebuilt at exactly 5% per slice on 200 cells to confirm `<= tolerance` semantics still pass.
...
- ✅ **RESOLVED (2026-05-09).** RUNBOOK's drill section rewritten around a prompt-weakening drill on `agent/src/graph/synthesize.prompt.ts` (the synthesizer source-id rule), replacing the legacy bbox-equality drill that targeted deterministic code. Drill #2 (MR !70 against the refactored gate, pipeline 4210) ran end-to-end: code-level jobs (`test:agent`, `test:dashboard`, `test:php-isolated`) all passed; only `test:agent-evals-gate` failed with a 16.8% pooled regression rate and five `(dataset, rubric)` slices over tolerance (briefingGraph + conversationalGraph × `citation_present`/`factually_consistent`, plus document-extraction `schema_valid`). `eval-gate-report.md` archived as a 90-day artifact (job 16539). The execution log now has its first real row.
...
- ✅ **RESOLVED (2026-05-09).** `CompareResult.removedBaselineCases` now lists every baselined case (post-skip) with zero live cells; `underTolerance` is gated on it being empty. The closed gap was: a case made up entirely of baseline-`false` cells could disappear silently because the cell-level missing-from-live check intentionally only punishes baseline-`true` cells. Pinned tests cover both the standard removal case and the baseline-`false` removal case, plus a regression where a vendor-skipped case is correctly *not* counted as removed.
...
- ✅ **RESOLVED (2026-05-09).** `runGate` now writes the rendered markdown to `agent/eval-gate-report.md` (configurable via a `reportPath` test seam) before returning; `.gitlab-ci.yml`'s `test:agent-evals-gate` job declares it as `artifacts.paths` with `expire_in: 90 days, when: always` so a gate-fail job (which exits non-zero) still uploads the report. Two integration tests against tmp dirs pin that the file lands on both pass and fail.
