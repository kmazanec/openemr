...
- USPSTF corpus ingest: one-shot `npm run grounding:reindex-corpus` script that reads curated USPSTF chunks from a versioned source (`agent/data/corpus/uspstf/`), embeds via OpenAI `text-embedding-3-large`, upserts to Pinecone with metadata `{publication, year, section, url, license_tier}`, namespace `guidelines-v1`. Idempotent re-runs.
