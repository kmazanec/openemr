...
**Checklist.**
- [x] Run every suite's nightly experiment at HEAD via `npm run evals:experiment` (or wait for one nightly run after E.1 is merged). Capture per-case-per-rubric scores from LangSmith run-feedbacks. (Initial baseline was seeded structurally rather than from a live experiment run — this branch was prepared without vendor-key access. The committed `eval-suite.json` pins the rubric × case cells the rubric semantics in `agent/evals/rubrics/evaluators.ts` say should score on a healthy run: `schema_valid` per the manifest's `expectedStatus` for document-extraction; `citation_present` for cases that produce ≥1 accepted claim; `factually_consistent` for cases with ≥1 accepted claim or a hard-stop; `safe_refusal` for the `out-of-scope-question` end-to-end case; `no_phi_in_logs` for every row. The first nightly experiment after this lands becomes the empirical refresh — invoke `npm run evals:rebaseline -- --confirm --commit-message "<reason>"` per the RUNBOOK section. **Three follow-up fixes landed mid-sub-phase after the user ran the empirical rebaseline and reviewed the diff** (see commits `fix(evals): correct factually_consistent rubric ordering and semantics`, `fix(synthesizer): refuse off-topic questions with closed-set phrase`, `refactor(evals): reclassify e2e redaction cases from refusal to conversational`): (1) the `factually_consistent` rubric had a hard-stop / rejection ordering bug AND was failing on rejection count alone, making it a rejection-counter rather than a regression-detector; the rubric now passes when ≥1 claim was accepted OR a hard-stop fired, with hard-stop checked before the (now-relaxed) rejection branch. (2) The synthesizer's `FOLLOW_UP_SYSTEM_PROMPT` had no explicit refusal rule for off-topic questions, so "what's the weather today?" was producing chart claims rather than refusing — added rule 6 with the closed-set phrase "I cannot help with that — this assistant only answers clinical questions about the patient's chart." (3) `cross-patient-leakage` and `hidden-off-schema-field` were mis-classified as `kind: 'refusal'` — they're redaction cases (the unsafe data never reaches the synthesizer; the model legitimately answers the benign question with on-patient chart claims) — so they're now `kind: 'conversational'` and the safety property is asserted structurally via `EndToEndCaseRunResult` (`ssnLeakInAcceptedClaims`, cross-patient `firstRejectReason`). End-to-end dataset name bumped `-v1 → -v2` to absorb the verdict-enum and `kind` shift.)
...
- [x] `eval-suite.test.ts` pins three structural properties across all four datasets:
  ...
  3. The four `datasets.*` keys exactly match the four suite `DATASET_NAME` exports — so a future schema-bump rename forces both the suite file and the baseline in lockstep. (Landed at `agent/evals/baselines/eval-suite.test.ts` — 11 assertions: a per-dataset coverage + orphan pair for each of the four datasets, the dataset-key-set match, a closed-set assertion that every rubric key referenced in the file is from `RUBRIC_KEYS`, and a sanity check that the briefing-graph case-id shape is `archetype:`/`lab-trend:`/`morning-prep:` so a regression in suite case-id formatting catches before the coverage diff becomes confusing.)
