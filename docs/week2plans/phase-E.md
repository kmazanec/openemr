...
- Every suite (`briefingGraph`, `conversationalGraph`, `documentExtraction`) runs its nightly experiment against real models with the boolean rubrics scored uniformly via `evaluators: [...RUBRICS]`. (The previous end-to-end suite was consolidated into `conversationalGraph`; its behavioral coverage now lives there.)
...
- [x] All cases use real models in CI per architecture §"Real model in CI for all 50 cases". (The per-MR Vitest gate stubs the LLM seam — the deterministic-gate convention used uniformly across `conversationalGraph` and `briefingGraph` Vitest cases. Real-model coverage of the multi-retriever and cap-hit invariants lives in the `conversationalGraphSuite` nightly LangSmith experiment alongside the document-evidence, guidelines, and verification rows; the five W2 boolean rubrics score every row uniformly via `evaluators: [...RUBRICS]` in `evaluate(...)`. Dataset name bumped to `clinical-copilot-conversational-graph-v2` because the case-group enum widened.)
...
**Goal.** A single baseline file pins per-case, per-rubric booleans across every eval dataset. The CI gate (E.4) treats all three suites as one rubric universe: it sums every scored cell across `briefingGraph`, `conversationalGraph`, and `documentExtraction`, compares against the baseline, and fails the build when more than 5% of cells flip from `true` to `false`. (The previous end-to-end suite was consolidated into `conversationalGraph`; its behavioral coverage now lives there.) Rebaselining is a deliberate, documented step — never a side effect.
...
**Checklist.**
- [x] Run every suite's nightly experiment at HEAD via `npm run evals:experiment` (or wait for one nightly run after E.1 is merged). Capture per-case-per-rubric scores from LangSmith run-feedbacks. (Initial baseline was seeded structurally rather than from a live experiment run — this branch was prepared without vendor-key access. The committed `eval-suite.json` pins the rubric × case cells the rubric semantics in `agent/evals/rubrics/evaluators.ts` say should score on a healthy run: `schema_valid` per the manifest's `expectedStatus` for document-extraction; `citation_present` for cases that produce ≥1 accepted claim; `factually_consistent` for cases with ≥1 accepted claim or a hard-stop; `safe_refusal` for refusal cases; `no_phi_in_logs` for every row. The first nightly experiment after this lands becomes the empirical refresh — invoke `npm run evals:rebaseline -- --confirm --commit-message "<reason>"` per the RUNBOOK section. **Two follow-up fixes landed mid-sub-phase after the user ran the empirical rebaseline and reviewed the diff** (see commits `fix(evals): correct factually_consistent rubric ordering and semantics`, `fix(synthesizer): refuse off-topic questions with closed-set phrase`): (1) the `factually_consistent` rubric had a hard-stop / rejection ordering bug AND was failing on rejection count alone, making it a rejection-counter rather than a regression-detector; the rubric now passes when ≥1 claim was accepted OR a hard-stop fired, with hard-stop checked before the (now-relaxed) rejection branch. (2) The synthesizer's `FOLLOW_UP_SYSTEM_PROMPT` had no explicit refusal rule for off-topic questions, so "what's the weather today?" was producing chart claims rather than refusing — added rule 6 with the closed-set phrase "I cannot help with that — this assistant only answers clinical questions about the patient's chart.")
- [x] Commit `agent/evals/baselines/eval-suite.json` with the shape:
  ```json
  {
    "version": 1,
    "committedAt": "2026-...",
  ...
  ```
  Each case row's keys are the rubric names that scored on that case (N/A rubrics omitted, mirroring the evaluator's skip semantics). The values pin the expected boolean — typically `true` at baseline, but a row can baseline a deliberate `false` (e.g. a refusal case where `safe_refusal` is `true` but a non-applicable structural rubric isn't even present). (Landed at `agent/evals/baselines/eval-suite.json` covering the consolidated three datasets: briefing-graph, conversational-graph, and document-extraction rows. `committedAt` is set to the seed date; `commitSha` is `null` until the rebaseline script overwrites the file with empirical data from a real experiment.)
- [x] `agent/scripts/rebaseline.ts` reads the latest LangSmith experiment's run-feedbacks for each dataset and writes the baseline file. Implementation note: use `langsmith.Client.listRuns({experimentName})` and follow each run's feedback edges; do not re-run the experiment locally. Aborts unless invoked with `--confirm` and a `--commit-message` argument that gets recorded in the commit body. (Landed at `agent/scripts/rebaseline.ts`; `parseArgs` and `caseIdFromRun` are exported and unit-tested at `agent/scripts/rebaseline.test.ts` (13 cases). The script resolves the latest experiment per dataset via `Client.listProjects({referenceDatasetId})`, streams runs via `Client.listRuns({projectName, isRoot: true})`, then pulls feedback in one batch via `Client.listFeedback({runIds})`. A `--commit-message=<text>` equals form is also accepted. The exported `rebaseline()` function takes an `experimentNameByDataset` override for one-off scripts that need to pin a specific experiment.)
- [x] `eval-suite.test.ts` pins three structural properties across all three datasets:
  ...
  3. The three `datasets.*` keys exactly match the three suite `DATASET_NAME` exports — so a future schema-bump rename forces both the suite file and the baseline in lockstep. (Landed at `agent/evals/baselines/eval-suite.test.ts` — a per-dataset coverage + orphan pair for each dataset, the dataset-key-set match, a closed-set assertion that every rubric key referenced in the file is from `RUBRIC_KEYS`, and a sanity check that the briefing-graph case-id shape is `archetype:`/`lab-trend:`/`morning-prep:` so a regression in suite case-id formatting catches before the coverage diff becomes confusing.)
- [x] Rebaseline procedure documented in `RUNBOOK.md`: when to rebaseline (model upgrade, intentional rubric tightening, deliberate suite expansion), how to invoke the script with `--confirm` + `--commit-message`, how to commit (separate PR labeled `eval-rebaseline`), how to record context (commit message describes what changed and why). (Added a `## Rebaselining the eval suite` section to `docs/RUNBOOK.md` covering when, how, and the explicit anti-pattern of rebaselining away a real CI failure.)
- [x] Delete the legacy per-suite baseline (`document_extraction_v1.{json,test.ts}`). The unified file is the single source of truth.

**Definition of done.** `agent/evals/baselines/eval-suite.json` committed at HEAD covers every case across all three datasets with per-rubric booleans; the structural test enforces manifest ↔ baseline correspondence across all three; rebaseline script works against a feature branch; runbook section is concrete enough for a second engineer to re-run without questions; legacy per-suite baseline is gone.
...
**Checklist.**
- [x] Document the drill procedure in `RUNBOOK.md`:
  ...
  (Landed in `docs/RUNBOOK.md` §"Regression-injection drill" — covers what the drill weakens with a permalink-friendly file:line target, the two failure signals (deterministic Vitest + real-model eval gate), step-by-step procedure, revert path if a drill commit accidentally lands on master, and an audit-log table for drill executions over time.)
- [ ] Execute the drill once before submission. Owner: User. (Procedure is fully documented and ready to run; the engineer wiring this PR did not run the drill — execution happens after the eval-gate CI plumbing merges, on a separate `drill/regression-injection-1` branch per the runbook procedure.)
...
- [ ] Record the drill execution in `RUNBOOK.md` with date, commit hash of the weakening, and CI pipeline URL. Owner: User. (Audit-log table is already pinned in `RUNBOOK.md` with a `_pending_` row; user fills it in after the drill completes.)
...
**Checklist.**
- [x] **Cost precheck (`check-cost-cap.ts`):** estimate the run cost (case count × model + embedding + rerank cost) before launching. Fail the job with a typed error if estimate > $5 (the architecture's per-PR hard cap). (Landed at `agent/scripts/check-cost-cap.ts`. `PER_CASE_USD` pins per-suite unit prices; the script multiplies by live `buildExamples` case counts and exits 1 with a typed error if the sum exceeds `HARD_CAP_USD = 5.0`. At HEAD: 29 briefing × $0.04 + 5 conversational × $0.10 + 26 doc-extraction × $0.06 + 6 end-to-end × $0.15 = $4.12 — comfortably under cap. Wired as `npm run evals:check-cost`. Six unit tests at `agent/scripts/check-cost-cap.test.ts` pin the multiplication, the boundary at $5, the over-cap branch, and the live-dataset path.)
- [x] **Vendor health check (`vendor-health-check.ts`):** GET each vendor's status endpoint (Anthropic status, OpenAI status, Cohere status, Pinecone status). If any vendor is down: emit a structured `vendor-outage` warning artifact, mark relevant cases as "skip with warning" in the run, continue with the rest. The skipped cases are flagged on the PR but don't fail the gate. Document in `RUNBOOK.md` what counts as "down". (Landed at `agent/scripts/vendor-health-check.ts` covering Anthropic, OpenAI, Cohere, Pinecone, **and LangSmith** — LangSmith was not in the original list but is the experiment recorder, so an outage there breaks the gate's read-back path. Hits each vendor's `/api/v2/status.json` endpoint with an 8-second timeout in parallel; any non-2xx, network error, or timeout marks the vendor `degraded`. `SUITE_VENDOR_DEPENDENCIES` maps each suite to its vendors so the gate knows which cases to skip. Five unit tests at `agent/scripts/vendor-health-check.test.ts` cover the all-ok, non-2xx, network-error, endpoint-override, and dependency-mapping paths. RUNBOOK §"Eval-gate CI job" defines "down" as any of: HTTP non-2xx, connection error, hard timeout at 8s.)
- [x] **Eval gate (`eval-gate.ts`):** (Landed at `agent/scripts/eval-gate.ts`.)
  1. Run every suite's experiment target against real models (one experiment per suite, posted to LangSmith with the PR's commit SHA). (`runExperiment` is now parallel — `Promise.all` over `SUITES` in `agent/evals/runners/experiment.ts`. Wall-clock dominated by slowest suite, not sum.)
  2. Pull per-rubric scores back from each experiment via `langsmith.Client.listRuns({experimentName})` + run-feedback edges. (`collectLiveCells` mirrors `rebaseline.ts`'s read pattern: enumerate runs by `projectName`, batch-fetch feedback by `runIds`, decode case ids via the shared `caseIdFromRun`.)
  3. Compare against `agent/evals/baselines/eval-suite.json` (the unified per-case-per-rubric baseline from E.2). (`compareLiveAgainstBaseline` is a pure function — fully unit tested.)
  4. Compute the regression rate as `flippedCells / totalScoredCells`, where the sum spans every applicable rubric × case across all four datasets. Fail the gate when `flippedCells / totalScoredCells > 0.05`. (Tolerance pinned at `REGRESSION_RATE_TOLERANCE = 0.05`. Boundary semantics: `<= 0.05` passes; `> 0.05` fails. Silent disappearance — a baselined `true` cell with no live counterpart — counts as flipped. Drift — a live cell whose `(dataset, caseId, rubric)` triple isn't in the baseline — fails the gate as `unknownLiveCells` even at 0% regression, since the structural test in `eval-suite.test.ts` is supposed to keep that pinned at commit time.)
  5. Vendor-outage skips don't count toward `flippedCells` or `totalScoredCells` — they're surfaced separately on the PR comment. (`resolveSkippedCases` builds the skip set from `checkVendorHealth().reports` and `SUITE_VENDOR_DEPENDENCIES`; skipped `(dataset, caseId)` pairs are excluded from BOTH numerator and denominator.)
  6. Emit a structured summary on PR (markdown comment via the GitLab API) with the regression rate, the list of flipped cells (`<dataset>::<caseId>::<rubric>`), and the vendor-outage skip list. (`renderMarkdownReport` produces the body; `postGitlabComment` posts to `POST /projects/:id/merge_requests/:iid/notes` using the `CI_JOB_TOKEN` GitLab supplies automatically. Skips silently when not in an MR pipeline so the script also works locally.)
- [x] **`.gitlab-ci.yml` job:** new `evals:gate` job (depends on `test:agent`), runs after build + lint pass. PR-blocking. Uses GitLab's `rules: changes` so it doesn't run on docs-only PRs (per W1 pattern). (Landed as `test:agent-evals-gate` with `needs: [test:agent]` and `rules: changes: [agent/**/*, .gitlab-ci.yml]`. Pass-through env vars cover all five vendor keys plus the GitLab-MR-context vars. The job runs `npm run evals:check-cost && npm run evals:upload-dataset && npm run evals:gate` — cost-precheck first, then the upload (idempotent), then the gate.)
- [x] Tests: stubbed-model unit tests for the gate script's pass/fail logic. Inputs are synthetic baseline + live-experiment shapes (no real LangSmith call). Assert exit code 0 on a 4%-flip rate (within tolerance), exit code 1 on a 6%-flip rate (over tolerance), exit code 1 on any unrecognized live case ID (the live run drifted off the baseline's tracked-case set), exit code 1 on a baseline `true` cell that's missing from the live run (silent disappearance is a regression too). (Landed at `agent/scripts/eval-gate.test.ts` — 18 cases. Covers: 4% → pass, 5% → pass at boundary, 6% → fail, missing-from-live → flipped, drift → fail at 0% regression, vendor-outage skips remove cells from numerator AND denominator, baseline-false stays not-a-flip, the `resolveSkippedCases` mapping per suite, and the markdown-report rendering. The orchestration layer (`runGate`) is exercised in CI rather than unit-tested — its dependencies are stubbed at the seam (the runner, the LangSmith client, the cell collector) but a full happy-path unit test would just be re-asserting the pure-function behaviors that are already pinned.)
