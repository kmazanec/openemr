...
**Files touched.**
- `agent/evals/cases/integration/*.test.ts` (new).
- `agent/evals/cases/no-phi/vision-traces.test.ts` (new) — extends the existing W1 phiTraceScanner pattern.
- `agent/evals/runners/documentExtractionSuite.ts` — extend.

**Checklist.**
- [ ] **Multi-retriever turn cases (~3 cases):** turn that genuinely needs both `documentEvidenceRetriever` and `evidenceRetriever`. Assert supervisor invokes both within the cap; final response cites both source types; rationale on each decision is non-empty.
- [ ] **Cap-hit cases (~2 cases):** state shaped so the supervisor would loop without the cap (e.g., retrievers always return empty). Assert `cap-hit` trace event emitted at iteration 10; forced-synthesize fires; response renders with appropriate Gap chips.
- [ ] **`no_phi_in_logs` vision-trace extension (~2 cases):** scan recent LangSmith vision-call traces for any patient identifiers from the test fixture (per `WEEK2-PRESEARCH.md` §W2-15 — vision payloads are PHI; `LANGSMITH_HIDE_INPUTS/OUTPUTS` defaults should suppress them). Extends the W1 §6.1 phiTraceScanner pattern. Reference: `agent/evals/cases/.../<existing W1 phiTraceScanner test>` for the pattern.
- [ ] **Drill regression case (~1 case):** the deliberate-regression fixture for the regression-injection drill (E.3). This test asserts the verifier's bbox-match requirement is enforced — when the drill weakens it, this case is the one that flips red.
- [ ] All cases use real models in CI per architecture §"Real model in CI for all 50 cases".

**Definition of done.** Total case count = 50 (8+8+6+4 pipeline + 4+4+6+4 conversational + 3+3 e2e + ~5–8 integration). Suite manifest in `documentExtractionSuite.ts` matches case-count distribution table in `W2_ARCHITECTURE.md` §"Case distribution". `npm test` green.
