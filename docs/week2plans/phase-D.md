...
- [x] Run `npm run grounding:reindex-corpus` against the deployed Pinecone (one-shot from the Droplet).
...
- [x] **Mrs. Patel scenario (3 cases):** full flow chart + lab PDF + intake form → briefing groups facts by `source_type`; citations present per claim; response coherent. Three cases differ in: (1) lab + chart only; (2) intake form + chart only; (3) lab + intake + chart all together.
  (Originally implemented under `agent/evals/cases/end-to-end/patel-scenario/` (3 cases) plus a sibling `_helpers.ts` (Mrs. Patel chart baseline + lab/intake `ExtractedFactSnippet` builders + ADA glycemic-targets guideline snippet + thin-slice driver). Per-MR layer fed verifier+format directly with hand-rolled drafts; the three cases pinned (a) chart+document grouping, (b) chart+document grouping with intake instead of lab, (c) the headline three-section render with two document cards (one per `document_uuid`) and a guideline section. Live-stack coverage ran through a dedicated suite against real Anthropic + Pinecone + Cohere + OpenAI in the nightly LangSmith experiment. The suite was later consolidated into `conversationalGraph`; the behavioral coverage now lives there.)
...
- [x] All 6 cases use real Anthropic Sonnet 4.x, real Pinecone, real OpenAI embeddings, real Cohere.
  (Real-vendor coverage originally lived in a dedicated end-to-end suite — registered in `suites.ts` alongside the existing per-suite `EvalSuite` entries, with one LangSmith dataset row per case (3 Patel + 3 refusal). The `runExperiment` adapter mirrored `conversationalGraphSuite`: it skipped with a structured reason when any of `PINECONE_API_KEY` / `PINECONE_INDEX_NAME` / `OPENAI_API_KEY` / `COHERE_API_KEY` was missing, and otherwise still skipped behind a "real-vendor experiment is gated until the corpus reindex on the deployed Pinecone index lands and a briefingRunner adapter is wired in" reason — the per-MR Vitest layer was the load-bearing structural gate. The suite was later consolidated into `conversationalGraph`; the dataset shape moved with it.)
...
**Definition of done.** 6 cases green. Suite registered. Trace metadata visible in LangSmith. (3 Patel + 3 refusal scenarios originally lived as 21 deterministic Vitest cases under a dedicated end-to-end cases directory; a dedicated end-to-end suite was registered in `suites.ts` with its own LangSmith dataset; the suite-registration test pinned the schema-bump suffix and the 6-example count. Live-vendor experiment was wired but skipped behind the corpus-reindex / briefingRunner-target follow-up. The suite was later consolidated into `conversationalGraph`; the behavioral coverage now lives there.)
