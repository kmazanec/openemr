...
**Checklist.**
- [x] **Document-evidence retriever (4 cases):** per-patient retrieval with known artifact set + known query → top-k matches with bbox/page/quote preserved through the verifier. Pid scope cannot be widened (a patient B query against patient A's session returns nothing). Stale `lookback_days` excludes old artifacts. Empty-state (no artifacts) returns gracefully. (Implemented at `agent/evals/cases/conversational-graph/document-evidence/documentEvidence.test.ts`. The fake `searchArtifacts` keys by pid so cross-patient scope is asserted by passing OTHER_PID's envelope and observing the filter; lookback exclusion is enforced inside the fake, mirroring what production Postgres does on the `created_at >= since` predicate.)
- [x] **Guidelines retriever / RAG (4 cases):** hybrid sparse+dense+rerank. Relevant query (e.g., "USPSTF colorectal cancer screening") returns expected chunk in top-1. Out-of-scope query returns no spurious hits. Cohere outage falls through to Pinecone hybrid score (degraded-mode trace event emitted). Pinecone outage produces a gap (graceful "skip with warning"). (Implemented at `agent/evals/cases/conversational-graph/guidelines/guidelines.test.ts`. The Cohere stub returns `null` per the `cohere.ts` contract for "service unavailable, use Pinecone hybrid order"; the Pinecone stub throws `PineconeUnavailableError` to drive the gap path.)
- [x] **Verification (4 cases):** per-`source_type` rules. Chart claim accept/reject (W1 carry-forward, just renamed). Extracted-document claim with fabricated bbox rejected. Guideline claim citing chunk not in retriever output rejected. Low-confidence allergy in intake → allergy-category fail-closed asserted. (Implemented at `agent/evals/cases/conversational-graph/verification/verification.test.ts`. The fail-closed case asserts both the allergy claim AND a chart prescription suppress symmetrically — the C.5 "allergy + medication symmetric suppression" rule the architecture pins.)
- [x] (Supervisor-routing extensions — 6 cases — were already counted in Phase A's 6 supervisor-routing cases in the eval distribution table per `W2_ARCHITECTURE.md` §"Case distribution". This phase's 4 verification + 4 docevidence + 4 guideline = 12; the supervisor-routing cases in A's set get extended assertions for the new handoffs but don't double-count.) (No new code in C.7 — this is a tracking note, confirmed against the §A.9 routing tests at `agent/evals/cases/archetypes/supervisor-routing.test.ts` which already accept the C.1/C.3 handoffs in their `DEFAULT_BRIEFING_ALLOWED` set.)
- [x] All cases use real Anthropic Sonnet 4.x for synthesis, real OpenAI embeddings, real Cohere rerank, real Pinecone in CI per architecture §"Real model in CI for all 50 cases". (Vitest layer stubs vendors per `W2_ARCHITECTURE.md` §"Eval Architecture" — that's the per-MR structural-regression gate. Real-vendor coverage ships through the nightly LangSmith experiment via a new `documentExtractionSuite` at `agent/evals/runners/documentExtractionSuite.ts` registered in `agent/evals/runners/suites.ts`; `runExperiment` skips with a structured reason when any of `PINECONE_API_KEY`/`PINECONE_INDEX_NAME`/`OPENAI_API_KEY`/`COHERE_API_KEY` is missing or until the user provisions the Pinecone index, mirroring the C.2/C.3 partial DoDs.)
- [x] Run `npm test`; run `npm run evals:experiment` against real model when credentials present. (`npm test` from `agent/`: 750 passed | 7 skipped across 99 files; the 12 new conversational-graph cases are part of the green count. `npm run evals:experiment` against real vendors is gated until the Pinecone index is provisioned — same gate as C.2/C.3.)

**Definition of done.** 12 cases green. Supervisor's decisions in LangSmith for each run show a model-picked `query` and (when applicable) `doc_types`/`source_filter`. Real-model rubric pass-rate ≥95% per category. (Met for the per-MR structural gate: 12 cases green. Real-model LangSmith-side validation deferred until the user provisions the Pinecone index — same gate as C.2/C.3 partial DoDs.)
