...
**Checklist.**
- [ ] Pick the AWS SDK v3 (Spaces is S3-compatible). Import `@aws-sdk/client-s3` and `@aws-sdk/s3-request-presigner`. Pin to a specific minor version.
- [ ] Implement `createSpacesClient({region, endpoint, accessKey, secretKey})` returning a thin wrapper exposing: `putObject`, `getObject`, `deleteObject`, `presignGetUrl(key, ttlSec)`, `presignPutUrl(key, ttlSec)`.
- [ ] Two clients are constructed at boot: an OpenEMR-side client (read+write, scoped to the bucket prefix) and an agent-side client (read-only on the transient prefix only). Document the IAM-policy expectations in a comment block at the top of the file.
- [ ] Helper functions: `keyForCanonical(pid, documentUuid, ext)` returns `s3://<bucket>/<pid>/<documentUuid>.<ext>`; `keyForTransientPage(documentUuid, pageNum)` returns `s3://<bucket>/<transientPrefix>/<documentUuid>/page-<pageNum>.png`.
- [ ] Tests: `agent/tests/storage/spaces.test.ts` — unit tests with the AWS SDK mocked. Verify TTL passed correctly to `presignGetUrl`. Integration test (skipped without credentials) that round-trips a small object.
- [ ] Add `SPACES_*` env vars to `agent/src/config/env.ts` with strict types (per CLAUDE.md "Parse, don't validate" rule).
...
**Checklist.**
- [ ] Define `PipelineState` per architecture: `{document_uuid, doc_type, pages: PageImage[], schema: ExtractionSchema | null, artifact_id: string | null, status, errors: PipelineError[]}`.
- [ ] Pick a rasterizer (per `WEEK2-PRESEARCH.md` Q4b — `pdf2pic` vs `pdfjs-dist + node-canvas` vs `pdf-img-convert`). Decision goes in a top-of-file comment with a one-line rationale. Wrap behind a `Rasterizer` interface so the choice is swappable.
- [ ] Implement `rasterize(state, deps): Promise<Partial<PipelineState>>`:
  1. Read PDF bytes from canonical Spaces key (constructed from `state.document_uuid` + `pid` from envelope).
  2. Pre-flight cost check: `pageCount * costPerPage > 1.00 USD` → return `{status: 'failed', errors: [{code: 'cost-cap-exceeded', message: ...}]}` (no rasterization performed).
  3. Render each page to PNG; upload each to the transient prefix with `keyForTransientPage(documentUuid, pageNum)`.
  4. Mint single-call ≤5-min-TTL signed GET URL per page via the agent-side client.
  5. Return `{pages: PageImage[]}` where each entry has `{pageNum, key, signedUrl, expiresAt}`.
- [ ] Cleanup: pipeline EXIT node (B.7) deletes transient objects on success or terminal failure; the 24h lifecycle policy is the backstop.
- [ ] Tests: stub the rasterizer + Spaces client; assert page-count cost cap fires on a 50-page fixture PDF; assert a 3-page PDF rasterizes and uploads.
...
**Checklist.**
- [ ] Define `agent/src/pipeline/schemas/labPdf.ts` Zod schema:
  ...
  - `.passthrough()` to silently drop unknown fields. Required fields are hard errors at parse time.
- [ ] Define `agent/src/pipeline/schemas/intakeForm.ts` Zod schema for: demographics, allergies, current medications (free-text → MedicationStatement-shaped), past medical history, family history.
- [ ] Mirror in PHP DTOs — same field names, same required/optional shape.
- [ ] Implement `vision(state, deps): Promise<Partial<PipelineState>>`:
  ...
  6. On rate-limit or transient error: retry once with backoff. On second failure or schema-invalid: return `{status: 'failed', errors: [{code: 'rate-limited' | 'schema_invalid', ...}]}`.
- [ ] PHI suppression: vision payload inputs/outputs are PHI; rely on the W1 `LANGSMITH_HIDE_INPUTS`/`HIDE_OUTPUTS` defaults (already set per `W2_ARCHITECTURE.md` §"Vision payloads"). Pino logger redaction extends to vision payload paths.
- [ ] Per-extraction trace metadata: `vision_input_tokens`, `vision_output_tokens`, `vision_dollar_cost`, `schema_validation_warnings`, `confidence_distribution` (histogram).
- [ ] Tests: stubbed-LLM unit tests covering happy path (returns valid schema), schema-invalid (Zod rejects), rate-limit retry-then-fail, and vision-injection-attack fixture (page contains "ignore previous instructions and ..." — assert the model still returns extraction-shaped output, not the injected instruction).
