...
**Checklist.**
- [x] **PHP endpoint:** `public/snapshot/extract.php` accepts POST `{pid, document_uuid, doc_type, trigger_source, canonical_ext?, conversation_id?}`. Departure from the plan as written: this endpoint is **browser-inbound**, not agent-inbound, so it uses the proxy pattern (session auth + outbound JWT mint via `AgentProxyController`), **not** the bearer-token `AgentEndpointAuth` pattern that other `snapshot/*.php` files use. The plan's "applies the existing AgentEndpointAuth" wording is incompatible with the architecture's trust direction (Path A is browser→OpenEMR→agent). The `extract` action is allowlisted in `PolicyGate` with the briefing read scopes plus `user/DocumentReference.cs` for the persist node's Tier-1 callback. `ExtractController` is a body-shape validator with a `Closure` dispatcher seam so isolated tests assert what the controller forwards without exercising the proxy's full machinery.
