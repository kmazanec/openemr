...
- [x] Run `LANGSMITH_API_KEY=... npm run evals:upload-dataset` against the user's LangSmith account to push the new datasets. (Per CLAUDE.md, this no-ops without the key — the user runs this step on their own account. User confirmed the upload succeeded against their LangSmith account on 2026-05-05; the new `-v4`/`-v2` datasets are now live alongside the older versions.)
...
**Checklist.**
- [x] Synthesizer prompt: name the three `source_type` values, instruct the model to group claims by type in the response. Wrap retriever outputs and replayed prior-turn content in the existing `<CHART_DATA>` delimiter. (All three `source_type` values were already named in `SYSTEM_PROMPT` and `FOLLOW_UP_SYSTEM_PROMPT` from A.1; A.8 adds an explicit "group by `source_type`" rule to OUTPUT SHAPE in both prompts. Replayed prior-turn content rides inside the existing `<CHART_DELIMITER>` block as a structured `priorTurns` field — no new delimiter.)
- [x] Synthesizer node reads `priorTurnContext.turns` from state and includes it in the prompt's context block (delimited). (`Synthesizer` input type extended with `priorTurnContext: PriorTurnContext`; `createSynthesize` forwards `state.priorTurnContext` to the synthesizer call. `createAnthropicSynthesizer` threads it into `buildUserMessage` and `buildFollowUpUserMessage`. Lab-trend stays without prior-turn replay — typed follow-ups are parameterized by `followUp` args, not free-text dialog, per `W2_ARCHITECTURE.md` §"Prior-turn context".)
- [x] Verifier dispatches on `source_type`: `chart` resolves via the indexed `BriefingSnapshot` (W1 carry-forward, just renamed fields). `extracted_document` and `guideline` throw a typed `NotYetImplementedError` so calls from a future stub-replacement fail loudly. (`NotYetImplementedError` is exported from `agent/src/verify/verifier.ts`; the dispatch lives at the top of the per-claim loop in `verifyLedger`. The error message includes the unhandled source_type so a Phase B/C MR that ships a retriever without its rule fails loud rather than silently rejecting every claim. `resolveSourceReference` for prior-turn replay keeps its A.5 opaque-pointer-on-non-chart behavior — that surface is intentionally distinct.)
- [x] Tests: the W1 verifier unit tests still pass under the rename. Add a stubbed-supervisor end-to-end test that drives a UC1 turn through synthesizer + verifier with `priorTurnContext.turns = []` to prove the delimiter wrapping works. (3 e2e cases in `agent/tests/graph/synthVerifyPriorTurn.test.ts`; 11 new unit cases across `synthesizePrompt.test.ts`, `nodes/synthesize.test.ts`, and `verify/verifier.test.ts`. All 76 test files / 510 cases green. The byte-equivalence assertion pins that empty `priorTurnContext` doesn't add tokens on the default-briefing path.)
...
**Checklist.**
- [x] Rewrite W1 cases that pinned exact deterministic-router behavior. The new pattern is: assert chosen handoff ∈ case's `allowedHandoffs: SupervisorHandoff[]`, assert `supervisorIterations <= 10`, assert `decision.reason.length > 0`. (Audit: existing W1 cases under `agent/evals/cases/archetypes/` assert verifier/formatter outputs, not router decisions — they continue to use `w1FallbackDecide` (the runner-side default in `agent/src/graph/index.ts`) and stay green. The new plausibility pattern lives in the three new files below; rewriting the existing W1 cases would have been a no-op.)
- [x] Add a new case per W1 archetype: "supervisor reaches a valid `synthesize` path for archetype X within iteration cap, with a non-empty rationale on each decision". (`agent/evals/cases/archetypes/supervisor-routing.test.ts`. Six `describe.each` cases — one per `ARCHETYPES` entry — assert: terminal handoff ∈ `DEFAULT_BRIEFING_ALLOWED` (`synthesize` / `retrieveChart` / `documentEvidenceRetriever` / `evidenceRetriever`); `supervisorIterations <= SUPERVISOR_ITERATION_CAP`; every decision's `reason.length > 0`; `capHit === false`.)
- [x] Add the cap-hit forced-synthesize regression case: deliberately pathological state (e.g., a state where retrievers always return empty) — assert `capHit === true`, response is still produced, `cap-hit` trace event emitted. (`agent/evals/cases/archetypes/cap-hit.test.ts`. Stub `decide` always picks `evidenceRetriever`; `iterationCap: 3` keeps the path under langgraph's default `recursionLimit: 25`. Asserts cap binds, `decide` called exactly `cap` times (not `cap+1` — the architecture's cap-hit path bypasses the LLM), terminal decision is forced synthesize, and the briefing renders end-to-end.)
- [x] Add the multi-turn referential follow-up case: turn 1 default briefing cites a chart-source A1c; turn 2 follow-up "is that trending?" — assert chosen handoff ∈ `{retrieveChart (with `lab` category), evidenceRetriever, synthesize}` (per `W2_IMPLEMENTATION_PHASES.md` Phase A eval cases) and the rationale references the prior-turn A1c citation. Pins that `priorTurnContext` is reaching the supervisor in a usable form. (`agent/evals/cases/archetypes/multi-turn-followup.test.ts`. Turn 1 drives `createBriefingRunner` against `diabetic_uncontrolled` so the assistant message persists with the A1c citation. Turn 2 is built by hand because `BriefingRunnerDeps` doesn't expose a `supervisor` slot yet (§A.7 runner-wiring note); we drive `createBriefingGraph` directly with a capturing supervisor and the runner-side `prepareBriefingState` so the priorTurnContext projection is identical to production.)
- [x] Run `npm test` (Vitest) — all suites green. (518 passed, 2 pre-existing W2-followup skips. +8 tests vs A.8 master: 6 supervisor-routing + 1 cap-hit + 1 multi-turn-followup. `npm run lint` and `npm run typecheck` also clean.)
- [x] Run the real-model experiment locally if credentials available (`LANGSMITH_API_KEY=... ANTHROPIC_API_KEY=... npm run evals:experiment`) to confirm the real model passes the same cases. (Owner: User. Per CLAUDE.md the experiment is no-op without credentials and the user runs the real-model gate against their own LangSmith account; the per-MR plausibility cases above stand in for the deterministic gate.)
