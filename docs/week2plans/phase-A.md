...
**Checklist.**
- [ ] Rename file + symbol. Update all imports.
- [ ] Add `retrieveChartCallCount` to `BriefingState` (default 0). Increment on entry to the node.
- [ ] First call (`callCount === 0`): run the existing W1 full fan-out (`getPatientContext`, `getPrescriptions`, `getRecentLabs`, `getRecentEncounters`, plus the W1 follow-up tools). Behavior unchanged from W1.
- [ ] Subsequent calls (`callCount > 0`): read `args.categories: ('diagnosis' | 'medication' | 'allergy' | 'lab' | 'encounter' | 'reminder' | 'medication_statement' | 'appointment')[]` from the supervisor's handoff. Fetch only the requested categories. Empty array is invalid → throw at the node entry (Zod-enforced upstream by the supervisor's args schema).
- [ ] Update the W1 cap-on-fanout / fail-closed-on-safety-critical / fail-open-on-informational tiered behavior — first call unchanged. Document that this behavior is W1 carry-forward in a single short comment.
- [ ] Tests: extend `agent/tests/graph/retrieve.test.ts` with a "subsequent call honors `args.categories`" case.
...
- `agent/src/graph/nodes/{kickoffExtraction,documentEvidenceRetriever,evidenceRetriever}.ts` — three new no-op stub nodes, each one returns control to supervisor with a logged "stub invoked" trace event.
- `agent/src/graph/index.ts` — wire the supervisor as the routing node. Conditional edges from supervisor → each handoff. `synthesize` is the only handoff that doesn't return to supervisor.
- `agent/src/graph/state.ts` — add `supervisorIterations: number`, `supervisorDecisionHistory: SupervisorDecision[]`, `capHit: boolean` slots.

**Checklist.**
- [ ] Define the supervisor's structured output schema in Zod:
  ```ts
  ...
  ```
- [ ] Implement the supervisor node: build the prompt from `BriefingState` (chart snapshot, envelope, accumulated retriever outputs, `priorTurnContext`, supervisor's own decision history this turn) + the handoff manifest (closed enum + brief description per handoff). Call Anthropic via `withStructuredOutput(SupervisorDecisionSchema)`. PHI-suppress the trace inputs per the W1 `LANGSMITH_HIDE_INPUTS` default.
- [ ] After each LLM call: append the decision to `supervisorDecisionHistory`, increment `supervisorIterations`, emit a structured LangSmith trace event per `W2_ARCHITECTURE.md` §"Per-supervisor-iteration trace event" — include `iteration`, `state_observed` (PHI-suppressed), `handoff_manifest`, `decision`, `rationale`, `args`, token counts, dollar cost.
- [ ] Wire conditional edges in the graph: each handoff routes back to supervisor *except* `synthesize`. The three new stub nodes (`kickoffExtraction`, `documentEvidenceRetriever`, `evidenceRetriever`) just emit a "stub invoked" trace event and return control to supervisor with no state changes — so the manifest is stable for B/C.
- [ ] Iteration cap: if `supervisorIterations >= 10`, force a transition to `synthesize` with `capHit: true` set on state. Emit a `cap-hit` trace event with the supervisor's last decision attempt and the state at termination.
- [ ] Cycle-detection: if the supervisor picks the same handoff twice in a row with no new retriever output between, emit a `degenerate-loop` warning trace event. Don't terminate — the iteration cap absorbs degenerate sequences.
- [ ] Tests: stubbed-LLM unit tests in `agent/tests/graph/supervisor.test.ts` covering: happy-path termination on `synthesize`, cap-hit forced synthesis, cycle-detection warning, malformed-LLM-output rejected by Zod (mock the LLM to return invalid JSON; assert the runner surfaces a typed error rather than passing it to the graph).
