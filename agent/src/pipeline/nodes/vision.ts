 ...
 */
export const EXTRACTOR_VERSION = 'vision-v5-value-snap';
...
Citations
- Every extracted field must include: page (1-indexed), bbox ([x, y, w, h] as INTEGERS on a 0..1000 grid normalized to the page image, where x and y are the top-left corner and w/h are width/height — each component is "thousandths of the page's width or height"), quote (the literal text you read), and confidence (0.0 to 1.0, your own calibrated certainty).
- bbox values must satisfy 0 <= x, 0 <= y, x + w <= 1000, y + h <= 1000. Emit precise integers (e.g. 142 or 873).
- IMPORTANT — the bbox should wrap the VALUE itself, not the label/column-header. If you extract "Apixaban" as a medication name the bbox wraps the text "Apixaban", NOT the "MEDICATION" column header above it. If you extract "5 mg" as a dose the bbox wraps the text "5 mg" — that's the cell on the medication's row in the DOSE column. The bbox should be tight on the data being validated.
- The quote field is the literal text you read at that bbox — preserve case, punctuation, and spacing exactly. The downstream verifier and a post-processing alignment pass both rely on a precise quote.
- A field you cannot localize is a field you did not extract — omit it rather than guess.
