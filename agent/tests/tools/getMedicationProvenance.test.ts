...
import { getMedicationProvenance } from '../../src/tools/getMedicationProvenance.js';
...
describe('getMedicationProvenance', () => {
    it('GETs the provenance endpoint with site + pid + medicationId', async () => {
        const { client, get } = mockAgentHttpResolving(narrowResponse);

        const out = await getMedicationProvenance({ client, ...callOpts() });

        expect(get).toHaveBeenCalledTimes(1);
        const call = get.mock.calls[0]?.[0] as { url: string; token: string };
        expect(call.token).toBe(TOKEN);
        expect(call.url).toBe(
            `${BASE}/interface/modules/custom_modules/oe-module-clinical-copilot/public/snapshot/medication_provenance.php?site=${SITE}&pid=${String(PID)}&medicationId=${String(MED_ID)}`,
        );
    ...
    it('throws on 401 (auth failure must surface, not become a gap)', async () => {
        ...
        await expect(
            getMedicationProvenance({ client, ...callOpts() }),
        ).rejects.toBeInstanceOf(AgentHttpError);
    });

    it('rejects a non-positive medicationId before any network call', async () => {
        const { client, get } = mockAgentHttpResolving(narrowResponse);
        await expect(
            getMedicationProvenance({ client, ...callOpts(), medicationId: 0 }),
        ).rejects.toThrow(/medicationId must be a positive integer/);
