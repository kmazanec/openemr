...
/**
 * main_v2.php — patient dashboard SPA shell.
 *
 * Near-copy of main.php that swaps the lower body block (attendantData
 * strip + tabs strip + frames-display) for a single mount point that
 * the dashboard SPA renders into. The top nav, search, user dropdown,
 * and notification dropdowns are preserved verbatim — only what lived
 * below the menu changes. Defended in PATIENT_DASHBOARD_MIGRATION.md
 * ("Integration architecture") and broken down in
 * docs/dashboard/build-plan.md (T3.1).
 *
 * Login routes here only when the user opted into the v2 shell at
 * login time (?v2=1 on the login page) or the OPENEMR_DASHBOARD_V2
 * environment variable is set on the openemr container. Without those,
 * main.php is the default.
 *
 * @package   OpenEMR
...
<head>
    ...
    <script>
        ...
        // T3.1: dashboard SPA reads window.erx_enable to decide whether
        // its Prescriptions card links to eRx.php (eRx-on flow) or to
        // the legacy controller.php?prescription page (eRx-off). T4.5
        // owns the consumer side.
        window.erx_enable = <?php echo OEGlobalsBag::getInstance()->getBoolean('erx_enable') ? 'true' : 'false'; ?>;
...
<body class="min-vw-100">
    ...
    <div id="mainBox" <?php echo $disp_mainBox ?>>
        ...
        <?php
        // Dashboard SPA mount point (T3.1). React renders the patient
        // identity bar, tab strip, and active tab content into this
        // single root, replacing the legacy attendantData/tabs_div/
        // framesDisplay blocks that lived here in main.php.
        //
        // The Vite build (dashboard/dist/) emits a hashed JS filename
        // per build; we read the manifest at request time so the
        // <script src="..."> tag points at the current build without
        // having to rerun PHP code generation. Failure to read the
        // manifest is rendered as a visible diagnostic so a missing
        // build doesn't fail silently.
        $manifestPath = $fileroot . '/dashboard/dist/.vite/manifest.json';
        $manifest = is_readable($manifestPath)
            ? json_decode((string) file_get_contents($manifestPath), true)
            : null;
        $entry = is_array($manifest) && isset($manifest['index.html']) && is_array($manifest['index.html'])
            ? $manifest['index.html']
            : null;
        ...
        <?php } else { ?>
            <div class="alert alert-danger m-3" role="alert">
                <?php echo xlt('Dashboard bundle not found.'); ?>
                <code><?php echo text($manifestPath); ?></code>
                — <?php echo xlt('run `npm run build` in dashboard/ or wait for the dev container to rebuild.'); ?>
            </div>
        <?php } ?>
